...
Avoiding default requires students to either obtain employment after graduation sufficient to make payment on their loans or place their loans in defferment while they attend additional schooling. The required payment is proportional to the amount owed. Given this, we hypothesize that the schools with high default rates will tend to be larger, less competative, privately controlled, and more expensive. 
...
```
Here we examine the distribution of several continuous variables(@fig-continuous-distributions). For each panel, we divided the observed values of our variable of interest into deciles and then display the empirical proportion of high default schools. There are several intersting observations in this figure:

* *Admission Rate appears unrelated to high default rate*. Across the deciles of `ADM_RATE` the lowest 10% (i.e. most competative) and highest 10% (i.e. those schools that accept nearly all applicants) have visually similar default rates.
...
```{r}
model <- glm(HIGH_CDR ~ CONTROL + (AVGFACSAL + GRAD_DEBT_MDN) + PCTFLOAN + PCTPELL +
               RET_FT4 + UGDS + HBCU + LOCALE + RELIGIOUS + ADM_RATE + 
...
```{r}
model_improved <- glm(HIGH_CDR ~ AVGFACSAL + RET_FT4 +  HBCU + PCTPELL + GRAD_DEBT_MDN, family = binomial(), data = train)
