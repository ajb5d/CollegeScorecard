...
Here we illustrate the impact of `CONTROL` on the relationship between various predictor variables and in-state tuition(@fig-cost-by-cont). It helps explain the "C" shape we were seeing in some of the earlier scatter plots, as the difference between Public and Private schools. It looks like the estimated slopes for some of these relationships will vary depending on whether the school is public or private. Specifically it looks like there may be a negative relation ship between `TUITIONFEE_IN` and `ADM_RATE`, `PCTPELL`, and `PCTFLOAN` when `CONTROL` is private. It looks like there may be a postivie realtionship between `TUITIONFEE_IN`and `AVGFACSAL` and `RET_FT4` that is greater when `CONTROL` is private rather than public.

```{r, echo=FALSE}
#| echo: false
#| fig-cap: "Tutition versus Continuous Variables, by Control"
