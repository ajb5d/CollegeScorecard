...
```{r, echo=FALSE}
initmodel <- lm(TUITIONFEE_IN~SATVRMID+AVGFACSAL+PCTPELL+RET_FT4+CONTROL, data = data)
summary(initmodel)
```
...
```{r, echo=FALSE}
impmodel1 <- lm(TUITIONFEE_IN~SATVRMID+AVGFACSAL+PCTPELL+RET_FT4+CONTROL+
                  SATVRMID:CONTROL+AVGFACSAL:CONTROL+PCTPELL:CONTROL+RET_FT4:CONTROL, data = data)
summary(impmodel1)
```

Based on the Adjusted $R^2$ value it looks like our model improved by adding the interaction terms. It jumped from 0.8241 previously to 0.8601. However it looks like RET_FT4 and the interaction term between RET_FT4 and CONTROL may be insignificant based of their t-test values. To confirm this we can run a partial F-test.

```{r, echo=FALSE}
impmodel2 <- lm(TUITIONFEE_IN~SATVRMID+AVGFACSAL+PCTPELL+CONTROL+
                  SATVRMID:CONTROL+AVGFACSAL:CONTROL+PCTPELL:CONTROL, data = data)
summary(impmodel2)
```
