...
The t-test for `ADM_RATE` indicates it may be possible to remove it from the model given all other regressors are included, but it needs to remain in the model since we are including an interaction term between `ADM_RATE` and `CONTROL`. Additionally the adjusted $R^2$ for this model is higher than all of the previous models.
...
Additionally the AIC values for the models also point towards the Recommended model as the best.
