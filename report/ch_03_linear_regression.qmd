...
We started off by generating scatter plots (@fig-corr) across the various variables to help show provide insight to some initial relationships. None of the predictor variables display an obvious linear relation ship to in-state tuition during this first pass of visualization. There are some predictors where the scatter plot looks to be creating a "C" shape. We will explore those variables further while also layering in some of those categorical variables later.
...
We illustrate the impact of `CONTROL` on the relationship between various predictor variables and in-state tuition(@fig-cost-by-cont) and this helps explain the "C" shape we were seeing in some of the earlier scatter plots, as the difference between Public and Private schools. It looks like the estimated slopes for some of these relationships will vary depending on whether the school is public or private. Specifically it looks like there may be a negative relation ship between `TUITIONFEE_IN` and `ADM_RATE`, `PCTPELL`, and `PCTFLOAN` when `CONTROL` is private. It looks like there may be a positive relationship between `TUITIONFEE_IN`and `AVGFACSAL` and `RET_FT4` that is greater when `CONTROL` is private rather than public.
