...
#DFBETAS<-dfbetas(logmodel)
#abs(DFBETAS)>2/sqrt(n)


res<-logmodel$residuals 
standard.res<-res/summary(logmodel)$sigma
student.res<-rstandard(logmodel) 
ext.student.res<-rstudent(logmodel) 
res.frame<-data.frame(res,standard.res,student.res,ext.student.res)

par(mfrow=c(1,3))
plot(logmodel$fitted.values,standard.res,main="Standardized Residuals")
plot(logmodel$fitted.values,student.res,main="Studentized Residuals")
plot(logmodel$fitted.values,ext.student.res,main="Externally  Studentized Residuals")
```
...
Unfortunately due to the log transformation on the response, `TUITIONFEE_IN` it is difficult to quantify the impact of changes in the predictors on the response in real world terms. However prediction is still viable.
